/*
    ...
    2. GET과 POST
        ...
        POST : 서버에 데이터를 전송하여 새로운 데이터를 생성하거나 기존 데이터를 변경할 때 사용하는 요청 방식으로, 회원 가입, 로그인, 글 작성, 파일 업로드 등에 사용됨. POST 요청은 데이터를 URL이 아니라 요청 본문(body)에 담아 보내기 때문에 주소창에 내용이 보이지 않으며, GET보다 보안성이 조금 더 높고 전송할 수 있는 데이터의 양에도 제한이 거의 없다.
    ...
    코드러너 실행: ctrl + option + n

    nodemon
    node.js 개발 시 자주 사용하는 유티리티로, 소스 코드가 변경될 때마다 자동으로 서버를 재시작해주는 도구
        npm install -g nodemon (모든 프로젝터에서 사용 가능, 권장 x)
    ...
    - 구조는 키-값 쌍으로 이루어진 객체 형태나 배열 형태를 사용
        ...
        const jsonStr = JSON.stringify(user) // 객체를 문자열로 변환 : 데이터를 편하게 주고 받기 하기위해서 문자열로 변환한다.
        
        cosnt jsonStr = "{ name: "김사과", age: 20 }"
...
const server = http.createServer((req, res) => {
    ...
    } else if (myUrl.pathname === "/api/user") {
