/*
    Express
    - node.js 환경에서 가장 널리 사용되는 웹 애플리케이션 프레임워크로, 서버를 쉽고 빠르게 구할 수 있도록 다양한 기능을 제공
    - 기본적인 HTTP 모듈보다 훨씬 간단하게 라우팅 처리, 요청/응답 객체 관리, 미들웨어 설정, 정적 파일 제공, 템플릿 엔진 연결 등을 할 수 있어 개발 생산성을 크게 높여줌
    설치방법 : npm install express || npm i express
    ...
    EJS
    - EJS(Embedded JavaScript)는 HTML안에 JavaScript 코드를 삽입해 서버 데이터를 동적으로 렌더링할 수 있게 해주는 node.js용 템플릿 엔진
    - <%= %>, <% %> 안에서 로직을 작성하여 사용
...
// static() : 정적 파일 사용
// use() : 미들웨어 등록
// app.use(express.static('public')) // root 접근
app.use("/static", express.static('public')) // static 접근, http://127.0.0.1:3000/static/spring.png
// post data 전송
...
app.get("/hello", (req, res) => {
    res.render("hello", { name: "김사과" }) // hello.ejs 파일을 찾는 코드
})
