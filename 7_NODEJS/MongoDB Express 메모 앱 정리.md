# MongoDB와 Express로 메모 앱 만들기

MongoDB는 대표적인 NoSQL 데이터베이스 중 하나로, 데이터를 테이블이 아니라 문서(Document) 형태로 저장한다.  
Express 서버와 MongoDB를 연결하면 클라이언트가 보낸 데이터를 데이터베이스에 저장하고, 다시 조회하거나 수정, 삭제하는 API를 만들 수 있다.

이번 글에서는 Express와 MongoDB를 이용해 메모 앱을 만드는 흐름을 정리한다.

1. MongoDB 개념
2. 문서 지향 데이터베이스
3. 스키마가 없는 구조
4. 요청 본문 Body
5. Express와 MongoDB 연결
6. 메모 생성, 조회, 검색, 수정, 삭제
7. URL 파라미터와 Query String
8. 프론트엔드에서 `fetch()`로 API 요청 보내기
9. 구현 시 주의할 점
...
MongoDB는 NoSQL 데이터베이스 시스템 중 하나다.  
관계형 데이터베이스처럼 데이터를 행과 열로 이루어진 테이블에 저장하지 않고, JSON과 비슷한 BSON(Binary JSON) 형식의 문서로 저장한다.
...
즉, MongoDB는 JavaScript 객체와 비슷한 형태로 데이터를 다룰 수 있어 Node.js와 함께 사용하기 좋다.
...
MongoDB에서는 여러 문서가 모여 컬렉션(Collection)을 이룬다.
...
```js
{ no: 2, userid: "banana", userpw: "2222", email: "apple@apple.com" }
```
...
MongoDB에 연결할 클라이언트 객체를 만든다.

---

## 9. 보안 주의: 접속 문자열은 공개하면 안 된다

MongoDB 접속 문자열에는 계정명, 비밀번호, 클러스터 주소가 들어간다.

```js
const url = "mongodb+srv://<username>:<password>@<cluster-url>/"
```

이 값은 절대 GitHub나 기술 블로그에 그대로 올리면 안 된다.  
외부에 공개되면 다른 사람이 데이터베이스에 접근할 수 있다.

실제 프로젝트에서는 `.env` 파일에 분리해서 관리하는 것이 좋다.
...
> 이미 접속 문자열을 공개한 적이 있다면 MongoDB 비밀번호를 변경하고, 가능하면 기존 접속 정보를 폐기하는 것이 좋다.
...
`$regex`는 MongoDB에서 문자열 패턴 검색을 할 때 사용하는 연산자다.
...
메모 앱에서는 새로 작성한 메모가 위에 보이도록 `-1`을 사용한다.
...
MongoDB는 저장할 때 자동으로 `_id` 필드를 추가한다.

---

## 19. insertOne()

`insertOne()`은 컬렉션에 문서 하나를 추가한다.
...
현재 예제에서는 `PUT /memos/:id`로 메모의 `text` 값을 수정한다.  
필드가 하나뿐인 단순 메모 앱이라 `PUT`으로 작성해도 이해하기 쉽다.

---

## 25. 메모 삭제: DELETE `/memos/:id`
...
삭제 결과는 `deletedCount`로 확인할 수 있다.
...
`deletedCount`가 `0`이면 조건에 맞는 문서를 찾지 못했다는 뜻이다.
...
`public/index.html`은 메모 앱 화면을 만든다.
...
현재 CSS 파일 첫 줄에 `css body {`처럼 작성되어 있다면 문법 오류다.  
CSS에서는 다음처럼 바로 `body` 선택자로 시작해야 한다.
...
전체 메모 조회 API는 브라우저에서도 확인할 수 있다.
...
```bash
curl -X POST http://127.0.0.1:3000/memo \
...
```bash
curl http://127.0.0.1:3000/memos
```

### 메모 검색

```bash
curl "http://127.0.0.1:3000/memos?keyword=MongoDB"
```

### 메모 수정

```bash
curl -X PUT http://127.0.0.1:3000/memos/메모_ID \
  -H "Content-Type: application/json" \
  -d '{"text":"수정된 메모"}'
```
...
```bash
curl -X DELETE http://127.0.0.1:3000/memos/메모_ID
```
