...
Express는 Node.js에서 웹 서버를 더 쉽고 구조적으로 만들 수 있게 해주는 프레임워크다.  
기본 `http` 모듈만 사용하면 요청 주소와 메서드를 직접 비교해야 하지만, Express를 사용하면 `app.get()`, `app.post()`, `Router` 같은 기능으로 서버 코드를 훨씬 깔끔하게 작성할 수 있다.

이번 글에서는 Express를 활용해 다음 내용을 정리한다.

1. 게시물 작성 폼 만들기
2. form 데이터를 받아 파일에 저장하기
3. 저장된 게시물 목록 보여주기
4. JSON 요청 처리하기
5. RESTful API 구조 이해하기
6. `app.route()`로 같은 경로의 요청 묶기
7. `express.Router()`로 라우팅 파일 분리하기
8. CommonJS와 ES Module 차이 이해하기

---

## 1. 게시물 작성 기능의 전체 흐름

게시물 작성 기능은 사용자가 브라우저에서 글을 작성하면, 서버가 그 데이터를 받아 파일에 저장하고, 저장된 게시물 목록 페이지로 이동시키는 구조다.
...
직접 문자열로 `"./data/post.txt"`를 작성할 수도 있지만, `path.join()`을 사용하면 macOS, Windows 같은 운영체제 차이에 더 안전하다.
...
HTML form에서 전송된 데이터를 `req.body`로 읽을 수 있게 해주는 미들웨어다.
...
`appendFile()`은 파일의 기존 내용을 지우지 않고, 뒤에 새로운 내용을 추가한다.
...
`<pre>` 태그는 줄바꿈과 공백을 입력된 그대로 보여준다.  
게시물 내용을 파일에 저장할 때 줄바꿈을 포함했기 때문에, `<pre>`를 사용하면 저장된 모양 그대로 출력할 수 있다.

---

## 8. JSON 요청 처리

HTML form이 아닌 JSON 데이터를 서버로 보내는 경우에는 다른 미들웨어가 필요하다.
...
브라우저 주소창은 보통 GET 요청만 보내기 때문에, JSON POST 요청은 Postman, Thunder Client, curl 같은 도구를 사용하면 편하다.
...
서버는 클라이언트가 항상 올바른 데이터를 보낸다고 믿으면 안 된다.  
...
여기서 `400`은 클라이언트 요청에 문제가 있다는 뜻이다.
...
수정이나 삭제처럼 이미 존재하는 자원을 처리한 경우에는 보통 `200` 또는 `204`를 많이 사용한다.

---

## 16. 모듈

모듈은 코드를 기능별 파일로 나누어 재사용할 수 있게 만든 구조다.  
...
`/posts`라는 같은 경로를 기준으로 GET, POST, PUT, DELETE 요청을 한 곳에 묶을 수 있다.
...
서버 실행 후 브라우저 또는 API 테스트 도구에서 아래 주소로 접속한다.

```text
http://localhost:3000
```

서버 종료는 터미널에서 `Ctrl + C`를 누른다.

> 같은 3000번 포트를 사용하는 서버 파일은 동시에 실행할 수 없다.  
> 다른 예제를 실행하려면 기존 서버를 먼저 종료해야 한다.
...
브라우저에서 게시물 작성 폼이 열린다.

```text
POST http://localhost:3000/posts
```

폼을 제출하면 게시물이 `data/post.txt`에 저장되고 `/posts`로 이동한다.
...
### 1. form 데이터와 JSON 데이터는 미들웨어가 다르다

```js
app.use(express.urlencoded({ extended: true }))
```

HTML form 데이터를 받을 때 사용한다.

```js
app.use(express.json())
```

JSON 요청 데이터를 받을 때 사용한다.

---

### 2. 응답 후에는 return으로 흐름을 끊는 것이 좋다
...
오류 응답을 보낸 뒤 아래 코드가 계속 실행되면 응답을 두 번 보내는 문제가 생길 수 있다.

---

### 3. `res.redirect()`는 새로운 GET 요청을 만든다

```js
res.redirect("/posts")
```

이 코드는 단순히 서버 내부 함수로 이동하는 것이 아니다.  
브라우저에게 `/posts`로 다시 요청하라고 알려주는 것이다.

---

### 4. RESTful API에서는 URL보다 HTTP 메서드가 동작을 표현한다
...
주소는 자원을 표현하고, 동작은 HTTP 메서드가 표현한다.

---

### 5. ES Module은 import/export를 사용한다
...
CommonJS 방식의 `require()`와 `module.exports`와 섞어서 사용하면 오류가 날 수 있으므로 파일 방식에 맞춰 작성해야 한다.

---

## 29. 정리

게시물 작성 예제는 Express에서 `form` 데이터를 처리하고, 파일에 저장하고, 저장된 데이터를 다시 화면에 보여주는 흐름을 연습하기 좋다.  
...
최종적으로 Express 서버 개발의 핵심은 다음 네 가지로 정리할 수 있다.
